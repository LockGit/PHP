...
如果数组内容非常大是不可以这么排序的，$save_arr 在达到指定的count之后需要清空$save_arr并释放内存，将排好序的内容写入硬盘，而$a,$b,$c,$d也是做演示，实际应该将文件分块以后
逐步对小的文件块排序并保存，在读取时也是依次读取适当的内存数据，会有频繁的磁盘读写，这个地方演示省去了。外排序即使文件再大通过分解最终合并也会完成排序，只是时间问题，当然相对
